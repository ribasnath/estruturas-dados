...
protected:
    ...
    Nodo<T>* buscarChaveDir(Nodo<T>* node, T chave) const {
        if (node != nullptr)
        {
            while (node != nullptr)
            {
    ...
    Nodo<T>* buscarChaveEsq(Nodo<T>* node, T chave) const {
        if (node != nullptr)
        {
            while (node != nullptr)
            {
    ...
    Nodo<T>* buscarChave(Nodo<T>* node, T chave) const {
        if (chave >= node->chave)
        {
            Nodo<T>* dir = buscarChaveDir(node, chave);
            if (dir != nullptr && chave == dir->chave) return dir;
        }
        else if (chave < node->chave)
        {
            Nodo<T>* esq = buscarChaveEsq(node, chave);
            if (esq != nullptr && chave == esq->chave) return esq;
        }
        return nullptr;
    };

    bool contemDir(Nodo<T>* node, T chave) const {
        if (node != nullptr)
        {
            while (node != nullptr)
            {
                if (chave == node->chave) return true;
                else if (chave < node->chave)
                    if (contemEsq(node, chave)) return true;
                node = node->filhoDireita;
            }
        }
        return false;
    ...
    bool contemEsq(Nodo<T>* node, T chave) const {
        if (node != nullptr)
        {
            while (node != nullptr)
            {
                if (chave == node->chave) return true;
                    ...
                    if (contemDir(node, chave)) return true;
                node = node->filhoEsquerda;
            }
        }
        return false;
    ...
    void insertDir(T chave, Nodo<T>* auxNode) const {
        while (auxNode != nullptr)
        {
            if (chave >= auxNode->chave && auxNode->filhoDireita != nullptr) {
                auxNode = auxNode->filhoDireita;
            }
            else if (chave >= auxNode->chave && auxNode->filhoDireita == nullptr) {
                Nodo<T>* newNode = new Nodo<T>{ chave };
                auxNode->filhoDireita = newNode;
                return;
            }
            else if (chave < auxNode->chave) {
                insertEsq(chave, auxNode);
                return;
            }
        }
        return;
    ...
    void insertEsq(T chave, Nodo<T>* auxNode) const {
        while (auxNode != nullptr)
        {
            if (chave < auxNode->chave && auxNode->filhoEsquerda != nullptr) {
                auxNode = auxNode->filhoEsquerda;
            }
            else if (chave < auxNode->chave && auxNode->filhoEsquerda == nullptr) {
                Nodo<T>* newNode = new Nodo<T>{ chave };
                auxNode->filhoEsquerda = newNode;
                return;
            }
            else if (chave >= auxNode->chave) {
                insertDir(chave, auxNode);
                return;
            }
        }
        return;
    ...
    void removerAux(Nodo<T>* nodePai, T chave) const {
        ...
        // se for sozinho
        /*else if (nodoRaiz->chave == chave && nodoRaiz->filhoDireita == nullptr && nodoRaiz->filhoEsquerda == nullptr) {
            delete nodoRaiz;
            nodoRaiz = nullptr;
            return;
        }*/
...
public:
    ...
    int quantidade() const override
    {
        ...
        */
        ...
        if (!vazia())
        {
            ...
            quant = (quantEsq(node) + quantDir(node)) + 1;
    ...
    void inserir(T chave) override
    {
        ...
        {
            Nodo<T>* auxNode = nodoRaiz;

            if (chave < auxNode->chave)
            {
                insertEsq(chave, auxNode);
            }
