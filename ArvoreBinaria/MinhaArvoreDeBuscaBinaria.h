...
protected:
    ...
    bool contemAux(Nodo<T>* node, T chave) const {
        if (node != nullptr) {
            if (chave == node->chave) return true;
            else if (chave > node->chave) contemAux(node->filhoDireita, chave);
            else if (chave < node->chave) contemAux(node->filhoEsquerda, chave);
        }
        else return false;
    }
